...
export const enumUsersSubscriptionPlan = pgEnum("enum_users_subscription_plan", ['free', 'starter', 'pro', 'vip']);
export const enumUsersSubscriptionPlanType = pgEnum("enum_users_subscription_plan_type", ['seasonal', 'lifetime']);
export const enumUsersSubscriptionStatus = pgEnum("enum_subscription_status", ['active', 'expired', 'cancelled']);
...
export const subscriptions = pgTable("subscriptions", {
    ...
    activeFrom: timestamp("active_from", { precision: 3, withTimezone: true, mode: 'string' }).notNull().defaultNow(),
    activeUntil: timestamp("active_until", { precision: 3, withTimezone: true, mode: 'string' }).notNull(),
